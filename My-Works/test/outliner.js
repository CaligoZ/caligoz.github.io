    ...
    if ($("#" + ioNode)[0] !=  null) {
        ...
        for (var i = 0; i < rowsBeingRevealed.length; i++) {
            ...
            if ($(rowsBeingRevealed[i]).hasClass("whole-note")) {
                ...
                else if (currentLevel <= previouslevel)="" {="" var="" foundparent="0;" previousrow="$(rowsBeingRevealed[i]).prev()[0];" locate="" the="" parent="" of="" row="" in="" question="" to="" see="" if="" should="" be="" visible="" or="" not="" while="" (foundparent="" !="1)" (($(previousrow).attr("data-level")="=" (currentlevel="" -="" 1))="" &&="" !($(previousrow).hasclass("whole-note")))="" ($(previousrow).hasclass("visible")="" ($(previousrow).attr("data-state")="=" "expanded"))="" $(rowsbeingrevealed[i]).removeclass("hidden").addclass("visible");="" }="" else="" save="" level="" and="" state="" current="" evaluate="" as="" next="" previouslevel="currentLevel;" ($(rowsbeingrevealed[i]).next()="" ($(rowsbeingrevealed[i]).next().attr("data-level")=""> currentLevel)) {
